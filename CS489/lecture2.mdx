...
$$\frac{dn}{dt} = \frac{1}{\tau_n(v)}{n_\inf(v) - n) \\ = \frac{1}{\text{time constant}}(\text{equilibrium value} - n)$$

- Larger time constant means it takes longer to get to equilibrium.

Typically written as $$\tau_n(v) * \frac{dn}{dt} = (n_\inf(v) - n)$$

### Sodium Channels
The fractionof sodium ion channels open is $$m^3h$$

$$\frac{dm}{dt} = \frac{1}{\tau_m(v)}(m_\inf(v) - m)$$
$$\frac{dh}{dt} = \frac{1}{\tau_h(v)}(h_\inf(v) - h)$$
...
- $$n_\inf + m_\inf$$ increase, while $$h_\inf$$ decreases
