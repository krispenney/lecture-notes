...
- WEP had the following design goals:
  ...
  - ...none of which held
  - **Spec**
    - sender and reciever share a secret key $$k$$
      - $$k$$ is either 40 or 104 bits long
      - Because of US restrictions
    - Transmit a message M
      - Compute a checksum: $$c(M)$$
      - Pick an IV v (24-bits), generate a keystream $$RC4(v, k)$$
      - Compute $$c = (M \mid\mid c(M)) \oplus RC4(v, k)$$
      - Send $$v \mid\mid c$$
        - note that v is sent as plaintext
    - Reciever gets $$v \mid\mid c$$
      - Recompute $$RC4(v, k)$$
      - $$M \mid\mid c(M) = c \oplus RC4(v, k)$$
  ...
  - Problems
    ...
    2. Checksum is CRC-32: not a good checksum for integrity guarantees
      - Linear Property: $$c(M \oplus D) = c(M) \oplus c(D)$$
      - issue: If they have the checksums for each part seperately, they can compute a valid checksum
      - The attacker can modify the message by flipping bits (D), computing that checksum, computing $$c \oplus (D \oplus c(D)) = (M \oplus c(M) \oplus (D \oplus c(D)) = (M \oplus D) \oplus (c(M) \oplus c(D))$$. This is a valid packet due to the linear property of the checksum.
